...
Usage:
    python analyze_mcce_proteins.py                  # analyze + write CSVs + print summary
    python analyze_mcce_proteins.py --plot           # plot only (requires prior analysis run)
...
FIG1_NAME = "residue_analysis.png"
...
def run_plot(outdir, topdir, target_ph, burial_threshold, dpi):
    ...
    # ── Figure 1: Residue analysis (3 columns) ──────────────────────────────
    fig1 = plt.figure(figsize=(18, 6))
    gs = gridspec.GridSpec(2, 3, height_ratios=[3, 1], hspace=0.05,
                           wspace=0.32, left=0.05, right=0.97, top=0.92, bottom=0.10)
    ax_violin = fig1.add_subplot(gs[:, 0])
    ax_burial = fig1.add_subplot(gs[:, 1])
    ax_scatter = fig1.add_subplot(gs[0, 2])
    ax_hist = fig1.add_subplot(gs[1, 2], sharex=ax_scatter)

    # Panel 1: pKa shift violin
    plot_data = [(r["restype"], r["pKa_shift_num"])
                 for r in residue_data if r["pKa_shift_num"] is not None]
    types_present = set(d[0] for d in plot_data)
    order = [rt for rt in IONIZABLE_ORDER if rt in types_present]
    colors = sns.color_palette("Set2", len(order))
    palette = dict(zip(order, colors))
    data_arrays = [np.array([s for t, s in plot_data if t == rt]) for rt in order]

    parts = ax_violin.violinplot(data_arrays, positions=range(len(order)),
                                  showmeans=False, showmedians=False, showextrema=False)
    for i, pc in enumerate(parts["bodies"]):
        pc.set_facecolor(palette[order[i]])
        pc.set_alpha(0.7)
    ax_violin.boxplot(data_arrays, positions=range(len(order)), widths=0.15,
                       patch_artist=True,
    ...
    ax_violin.axhline(0, color="gray", linestyle="--", linewidth=0.8, zorder=0)
    ax_violin.set_xticks(range(len(order)))
    ax_violin.set_xticklabels(order)
    ax_violin.set_ylabel("pKa shift  (pKa − pKa$_0$)")
    ax_violin.set_title("pKa Shift Distribution by Ionizable Residue Type")
    for i, rt in enumerate(order):
        n = sum(1 for t, _ in plot_data if t == rt)
        ax_violin.text(i, ax_violin.get_ylim()[1] * 0.95, f"n={n}",
                        ha="center", va="top", fontsize=7, color="gray")

    # Panel 2: Burial propensity bars
    ion_types = sorted([rt for rt in IONIZABLE_ORDER if rt in burial_pct],
                       key=lambda rt: burial_pct.get(rt, 0), reverse=True)
    non_ion_types = sorted([rt for rt in NON_IONIZABLE_ORDER if rt in burial_pct],
                           key=lambda rt: burial_pct.get(rt, 0), reverse=True)
    labels = ion_types + [""] + non_ion_types
    values = ([burial_pct.get(rt, 0) for rt in ion_types] + [0] +
              [burial_pct.get(rt, 0) for rt in non_ion_types])
    x = np.arange(len(labels))
    ion_color = sns.color_palette("Set2")[3]
    non_ion_color = sns.color_palette("Set2")[0]
    bar_colors = [ion_color] * len(ion_types) + ["white"] + [non_ion_color] * len(non_ion_types)
    ax_burial.bar(x, values, color=bar_colors, edgecolor="gray", linewidth=0.5)
    ax_burial.set_xticks(x)
    ax_burial.set_xticklabels(labels, rotation=45, ha="right")
    ax_burial.set_ylabel(f"% Buried  (frac. acc. ≤ {burial_threshold})")
    ax_burial.set_title("Burial Propensity: Ionizable vs Non-Ionizable Residues")
    ax_burial.legend(
    ...
    for i, (lbl, val) in enumerate(zip(labels, values)):
        if lbl:
            ax_burial.text(i, val + 1.0, f"{val:.0f}%", ha="center", va="bottom", fontsize=6.5)

    # Panel 3: pKa shift vs accessibility (scatter + histogram)
    all_frac = []
    for rt in order:
        ...
        if not subset:
            continue
        xs, ys = zip(*subset)
        ax_scatter.scatter(xs, ys, label=rt, alpha=0.5, s=12,
                           color=palette[rt], edgecolors="none")
        all_frac.extend(xs)
    ax_scatter.axhline(0, color="gray", linestyle="--", linewidth=0.8, zorder=0)
    ax_scatter.axvline(burial_threshold, color="red", linestyle=":", linewidth=0.8,
                       alpha=0.6, label=f"burial cutoff ({burial_threshold})")
    ax_scatter.set_ylabel("pKa shift  (pKa − pKa$_0$)")
    ax_scatter.set_title("pKa Shift vs Fractional Solvent Accessibility")
    ax_scatter.legend(fontsize=7, ncol=2, loc="upper right", framealpha=0.8)
    ax_scatter.set_xticklabels([])

    bins = np.arange(0, max(all_frac) + 0.1, 0.1)
    ax_hist.hist(all_frac, bins=bins, color="steelblue", edgecolor="white", linewidth=0.5)
    ax_hist.axvline(burial_threshold, color="red", linestyle=":", linewidth=0.8, alpha=0.6)
    ax_hist.set_xlabel("Fractional Solvent Accessibility")
    ax_hist.set_ylabel("Count")
    ax_hist.set_xlim(ax_scatter.get_xlim())
    ...
    # ── Figure 2: Protein charge analysis (1x3) ─────────────────────────────
    charge_col = f"net_charge_pH{int(target_ph)}"
...
def main():
    ...
    parser.add_argument(
        ...
        help="Generate figures (requires CSVs from a prior analysis run, or run without --plot first)"
    )
    parser.add_argument(
